...
final class WindowManager: ObservableObject {
    ...
    func adopt(_ window: NSWindow) {
        ...
        // Résolution ou moniteur changé : re-tuiler le workspace pour rester
        // calé sur le nouvel écran utile.
        NotificationCenter.default.addObserver(
            forName: NSApplication.didChangeScreenParametersNotification, object: nil, queue: .main
        ) { [weak self] _ in
            MainActor.assumeIsolated {
                guard let self, self.workspaceActive else { return }
                self.applyWorkspace()
            }
        }
    }
    ...
    // MARK: - Workspace : N fenêtres tuilées comme une seule app

    /// Le workspace courant. Le compagnon en fait TOUJOURS partie (tuile
    /// sentinelle en tête) : dans la disposition, c'est une app comme une autre.
    @Published var workspace = Workspace(tiles: [WorkspaceTile(bundleID: WorkspaceTile.companionBundleID)])

    /// Vrai une fois le workspace disposé : l'UI bascule alors ses libellés et
    /// re-tuile sur changement d'écran.
    @Published private(set) var workspaceActive = false

    /// L'app en cours d'exécution correspondant à une tuile (pour nom + icône
    /// dans l'UI). nil si l'app a été fermée depuis son ajout.
    func runningApp(for tile: WorkspaceTile) -> NSRunningApplication? {
        NSRunningApplication.runningApplications(withBundleIdentifier: tile.bundleID).first
    }

    // — Composition (pur état, sans toucher aux fenêtres → testable) —

    /// Ajoute un bundleID au workspace, sans doublon. Renvoie false si déjà là.
    @discardableResult
    func composeAdd(bundleID: String) -> Bool {
        guard !workspace.tiles.contains(where: { $0.bundleID == bundleID }) else { return false }
        workspace.tiles.append(WorkspaceTile(bundleID: bundleID))
        return true
    }

    /// Retire une tuile. Le compagnon, lui, est indéboulonnable : un workspace
    /// sans Claude n'aurait pas de sens pour cette app.
    func composeRemove(bundleID: String) {
        guard bundleID != WorkspaceTile.companionBundleID else { return }
        workspace.tiles.removeAll { $0.bundleID == bundleID }
    }

    // — Actions liées à l'UI —

    func addToWorkspace(_ app: NSRunningApplication) {
        guard let bundleID = app.bundleIdentifier else {
            dockError = "« \(app.localizedName ?? "?") » n'a pas d'identifiant exploitable."
            return
        }
        if composeAdd(bundleID: bundleID), workspaceActive { applyWorkspace() }
    }

    func removeFromWorkspace(bundleID: String) {
        composeRemove(bundleID: bundleID)
        if workspaceActive { applyWorkspace() }
    }

    func setWorkspaceAxis(_ axis: Workspace.Axis) {
        workspace.axis = axis
        objectWillChange.send()
        if workspaceActive { applyWorkspace() }
    }

    /// LE geste demandé : tuiler toutes les apps du workspace sur l'écran utile,
    /// compagnon compris — « maximiser l'app qui encapsule plusieurs apps ».
    ///
    /// À défaut de vrai plein écran (un Space = une app), on remplit
    /// `visibleFrame` ; barre de menus en masquage auto, c'est l'écran entier.
    func applyWorkspace() {
        dockError = nil
        guard let window else { return }
        ...
        guard Accessibility.isTrusted else {
            needsAccessibility = true
            Accessibility.requestTrust()
            dockError = "Autorisez l'Accessibilité pour disposer les fenêtres du workspace."
            return
        }
        needsAccessibility = false

        // Le workspace et l'ancrage à deux se disputeraient les fenêtres : un
        // seul régisseur à la fois. On sort proprement du mode paire.
        undock()

        let screen = ScreenGeometry.screen(containing: window.frame) ?? NSScreen.main
        guard let visible = screen?.visibleFrame else { return }

        let frames = workspace.frames(in: visible)
        guard !frames.isEmpty else {
            dockError = "Trop de fenêtres pour cet écran : \(workspace.tiles.count) apps ne tiennent pas côte à côte ici."
            return
        }

        isPinned = true // le compagnon doit rejoindre le Space courant
        var hadFullScreen = false
        for (tile, frame) in zip(workspace.tiles, frames) {
            if tile.isCompanion {
                window.setFrame(frame, display: true, animate: true)
            } else if place(bundleID: tile.bundleID, at: frame) {
                hadFullScreen = true
            }
        }
        workspaceActive = true

        // Sortir du plein écran natif prend ~0,5 s : la fenêtre n'accepte sa
        // nouvelle frame qu'après. On re-tuile une fois, une seule.
        if hadFullScreen {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.6) { [weak self] in
                self?.reapplyWorkspaceFrames(in: visible)
            }
        }
    }

    /// Place une fenêtre distante. Renvoie true si elle était en plein écran
    /// natif (il a fallu l'en sortir → re-tuilage différé nécessaire).
    @discardableResult
    private func place(bundleID: String, at frame: NSRect) -> Bool {
        guard let app = NSRunningApplication.runningApplications(withBundleIdentifier: bundleID).first,
              let win = AXWindow.mainWindow(of: app) else { return false }
        let wasFullScreen = win.isNativeFullScreen
        if wasFullScreen { _ = win.exitNativeFullScreen() }
        win.setFrame(frame)
        app.activate()
        return wasFullScreen
    }

    /// Deuxième passe après sortie de plein écran, sans re-déclencher la
    /// permission ni le undock (déjà faits).
    private func reapplyWorkspaceFrames(in visible: NSRect) {
        guard let window else { return }
        let frames = workspace.frames(in: visible)
        ...
        for (tile, frame) in zip(workspace.tiles, frames) {
            if tile.isCompanion {
                ...
                place(bundleID: tile.bundleID, at: frame)
            }
        }
    }
